...
def get_table_structure(img_imread, img_output):
    data_base_dir = img_imread  # 输入文件夹的路径
    outfile_dir = img_output  # 截取红章文件夹放置路径
    processed_number = 0  # 统计处理图片的数量
    for file in os.listdir(data_base_dir):  # 遍历目标文件夹图片
...
def print_xy(img):
    ...
    print('红章坐标：x轴,y轴：',circle_point[0])


#打印宽高
def pring_width_hight(img):
...
def print_u1(img):
    ...
    print('线条坐标：',img.shape[:2])
    # cv2.imshow('image1', img)
    # cv2.waitKey(0)s
    # cv2.destroyAllWindows()
pretreatment('D:/img_imread', 'D:/img_imread') #预处理
Eliminate_red_chapter('D:/img_imread', 'D:/img_output')# 消除红章
get_table_structure('D:/img_imread', 'D:/img_output2') # 获取表结构
extract_red_chapter('D:/img_imread', 'D:/img_output3')# 提取红章并打印出坐标位置
repair('D:/img_output','D:/img_output4')# 修复
